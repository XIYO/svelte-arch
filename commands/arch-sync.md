...
2. 이 플러그인의 `skills/svelte-arch/kit/sync.mjs`를 프로젝트 루트에서 실행 (`${CLAUDE_PLUGIN_ROOT}` 변수가 있으면 그 경로 기준):

   ```bash
   bun "<플러그인 루트>/skills/svelte-arch/kit/sync.mjs"
   ```
...
4. **구(비-FSD) 구조 감지 시** — sync 출력이 안내하는 대로:
   - SvelteKit 공식 기본 좌표(`src/routes`·`src/lib`·`src/app.html`·`src/hooks.*`)와 deprecated `kit.files.*` 잔존 여부를 확인한다. v5 커스텀 좌표가 남았으면 v6 마이그레이션이 출력한 매핑을 사용자에게 제시한다.
