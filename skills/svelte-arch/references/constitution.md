...
| **smart** (출처를 앎) | ∅ — 존재 금지 | `*.live` |

smart+무지 칸이 공집합인 이유: "데이터를 안다" = "어느 slice의 api를 부를지 안다" = 이미 업무 지식.

**용어 사전(업계 대응)**: view=presentational(Abramov)·recipe의 부품(Frost) / live=container·connected / shared/ui=design system component(Frost)·FSD shared/ui — ⚠ 업계 "primitives"(Radix·bits-ui)는 무스타일 headless로 다른 것(우리 스택에선 bits-ui가 그 층, shared/ui가 styled wrapper) / widget=FSD widget("자립적 대형 블록") / slice=FSD slice(도메인).
...
경계:      *.live → 자기 slice api(remote) · 하위 계층 api/ui — 서버 값 import는 remote·글루서버·endpoint·hooks만
...
**서버는 예외 0** — remote가 repository·adapter를 직접 부르지 않는다(얇은 pass-through service를 감수한다. 예외는 쌓여서 혼란이 된다). **근거**: 계층 격리 = 테스트 격리. live/remote는 런타임 결합부라 로직을 0으로 만들어(humble object) 테스트 가능한 층(view·service)으로 밀어낸다.

### A2. 전면 마킹 불변식

관장 트리(`src/**`, vendor·예약 파일 제외)의 모든 `.svelte`는 **글루**(`+page`/`+layout`/`+error`) · **`.view`** · **`.live`** · **`.stories`** 중 하나다. routes 콜로케이션도 예외 없음(무표 = `UNMARKED_COMPONENT`). 모든 `.ts`는 §3 종별 중 하나로 접미사(또는 지정 위치)를 갖는다(`UNMARKED_TS`).
...
| `*.live` / `*.remote` | 전용 불요 — 로직 0이 규칙 | e2e가 배선 커버 |
...
│   └── routes/                # 글루(+page/+layout) + pages first 콜로케이션(.view/.live 마킹)
...
live·view 마운트, 라우트 파라미터 추출(`$derived(page.params.x)`)·prop 전달, **Snippet 주입**(셸 위젯 view의 슬롯에 live를 끼움), `<svelte:head>`만. **금지**: `$state`·`$effect`·remote import(`GLUE_LOGIC`). view는 live를 마운트하지 않는다 — 셸이 데이터 위젯을 품을 땐 Snippet prop을 열고 글루가 주입.

### 3.2 routes 콜로케이션 — pages first의 1차 거처

라우트 폴더의 `+` 없는 파일. `.view`/`.live` 마킹 의무, `.ts`도 종별 의무. 둘째 라우트가 소비하려는 순간 하강 관문(discipline.md)으로.
...
- **허용**: 하위 계층 view(딥/index)·shared/ui·`import type`(wire — index 경유)·util·아이콘. **금지**: `.remote` 값(`REMOTE_IN_VIEW`)·`*.svelte.ts` 값(`STATE_MODULE_IN_VIEW`)·`$app/state`·`$app/navigation`(`APP_STATE_IN_VIEW` — `active` 등은 prop 주입)·`.live` import.
- **앵커**: `<!-- @component -->` 1행 + `type Props` 명명 + (shared/ui는 전 prop TSDoc). mutation은 콜백 `onXxx`로 위임.

### 3.4 `*.live.svelte` — smart (데이터 섬 배선)

- **정의**: remote를 **페어 view 1개**에 결합하는 humble 배선. 같은 폴더·같은 Base + `.live.svelte`(`LIVE_WITHOUT_PAIR`). **거주지**: routes 콜로케이션·widgets·features의 ui — **entities/ui 금지**(`ENTITY_UI_VIEW_ONLY`, live 욕구 = widget 승격 신호).
- **허용**: 페어 view · slice api(자기·하위 계층, index 경유) · model `*.svelte.ts` · `$app/navigation`·`$app/state`(URL 동기화는 live 소관) · shared/ui(pending/failed 표시용).
- **마크업 0**(`LIVE_MARKUP`): `<svelte:boundary>` + 페어 마운트 + pending/failed 스니펫만. 글루 외 import 금지(`LIVE_IMPORT_OUTSIDE_GLUE`).
...
- `*.svelte.ts` = runes 상태·상태기계. **소비 = live(와 글루)만** — view가 import하면 mock 격리가 깨진다. 무도메인 전역은 `shared/model/`.
...
- **데이터 수급 사다리**: ① live의 remote(기본) → ② `+page.ts` universal load(라우트 레벨 필요 — load에서 remote query await 가능) → ③ `+page.server.ts`(시크릿·server-only일 때만) → ④ `+server.ts`(한계 5종). 각 단계는 위 단계의 불가능을 말할 수 있어야 내려간다.
...
| 판정 (위에서부터) | 거주지 | 소유 |
|---|---|---|
| 링크 공유·새로고침·뒤로가기로 복원돼야 하는가 | URL searchParams | live (goto/replaceState — hydration 전 replaceState는 throw, 초기화 가드) |
| URL엔 과하지만 뒤로가기 제스처는 존중해야 하는가 | shallow routing `pushState`+`page.state` | live |
| 라우트를 떠나도 살아야 하는가 | `model/*.svelte.ts` | live만 소비 |
| 바뀌면 서버 재조회인가 (URL 불요 시) | live 로컬 `$state` | live |
...
| KnowledgeListSection + live | widgets/knowledge-list (자립 조립 블록) |
| AppSidebar·AdminSidebar·ToastHost | 셸 블록당 widget slice (widgets/app-sidebar 등) |
| "entity ui에 live를 두고 싶다" | widget 승격 신호 — entities/ui는 view 전용 |
