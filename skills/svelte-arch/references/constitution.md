...
**용어 사전(업계 대응)**: view=presentational(Abramov)·recipe의 부품(Frost) / container=container·connected(업계 관용어 표기와 합류 — v5에서 `.live` 폐기. **단 채택 근거는 "업계 표준이라서"가 아니다** — Abramov가 2015 원안을 2019 철회했으므로, 우리 근거는 remote/boundary 결합이 만드는 *기계적* 경계다. fsd-guide §6) / shared/ui=design system component(Frost)·FSD shared/ui — ⚠ 업계 "primitives"(Radix·bits-ui)는 무스타일 headless로 다른 것(우리 스택에선 bits-ui가 그 층, shared/ui가 styled wrapper) / widget=FSD widget("자립적 대형 블록") / slice=FSD slice(도메인).
...
**조직 원리 — 선형 tier가 아니라 inside/outside**: 이 서버 수직선은 엄격한 3-tier 서열로 못박지 않는다 — 헥사고날 원전의 **inside(코어: service 순수 로직) / outside(adapter: DB·외부 SDK) 비대칭**으로 읽는다(Cockburn: "there may be more than two ports … does not fit into the one-dimensional layer drawing"; 규칙 = *inside 코드가 outside로 새지 않는 것*). repository·adapter는 outside를 **port(인터페이스)로 추상화**한 driven adapter이며(→ §3.8·§3.9의 인터페이스 규범), 이 프레이밍이라야 위의 "테스트 격리"가 in-memory adapter 주입으로 **실제** 성립한다. 선형 controller→service→repository는 편의적 기본 배치이지 규칙의 본질이 아니다.
...
- **근거**: 배열 리터럴이어야 린터·정렬·감사·매니페스트가 클래스를 읽는다. **Svelte 5.16+가 clsx 시맨틱을 프레임워크에 내장**(공식: "class can be an object or array, converted to a string using clsx"; 5.19 `ClassValue` export)하므로 `clsx`·`classnames`는 **진짜 중복** — 금지 정당. **단 `tailwind-merge`는 다른 문제**(유틸 충돌 해소: `p-2`+`p-4`→뒤쪽 승)를 풀며 내장 배열은 이걸 **하지 않는다**. 즉 twMerge 금지는 "중복이라서"가 **아니라** "충돌 해소를 variant 우선·REPLACE형 해치 규율로 **대체하는 의식적 트레이드오프**"다 — 대체가 성립하려면 그 규율이 실제로 지켜져야 한다(주류 shadcn-svelte는 여전히 `cn=twMerge(clsx())`을 씀).
...
| `*.service` / `*.repository` | vitest **의무** (service 팩토리는 port 더블 주입으로 격리, repository는 테스트 DB 통합) | vitest |
...
- **boundary 한계**(Svelte 공식, 5.3.0+): `<svelte:boundary>`는 **렌더·effect 중 에러만** 포착 — 이벤트 핸들러·`setTimeout`·async 밖 에러는 **미포착**(그쪽은 명시적 try/catch로). SSR은 기본 무효(렌더 에러 시 전체 실패; 서버 처리는 5.51 `transformError`). `failed`/`onerror`가 있으면 기존 콘텐츠는 제거된다.
...
- ⚠ **성숙도(2026-07 기준)**: remote functions는 SvelteKit **experimental** 기능이다 — 2.27.0 도입, `kit.experimental.remoteFunctions`+`compilerOptions.experimental.async` opt-in, 최신 릴리스까지 stable 미승격, 공식 문서 "subject to change without notice". API가 minor 사이 breaking하게 바뀐 이력(`.run()` 2.56 추가→2.61 제거). → **소비 프로젝트는 SvelteKit 버전을 고정**하고 신규 기능부터 점진 채택. 이 계층 위의 룰들은 그 전제 위에 선다.
- **얇음의 책임 목록**: 인증 가드(`*.guard`) · **입력 검증(Standard Schema — Zod/Valibot. 각 remote는 공개 HTTP 엔드포인트라 미검증 = 신뢰 경계 붕괴. 실제 DoS 실증 사례 존재)** · **service 호출만**(`REMOTE_SKIPS_SERVICE` — repository·adapter 직접 금지) · 전송 매핑. db/schema/drizzle 값 import 금지(`REMOTE_DB_IMPORT`).
...
### 3.8 `*.port.ts` / `*.service.ts` / `*.repository.ts` — 서버 로직 (port 우선)

- **`*.port.ts` = driven port(계약 인터페이스)**. repository/adapter가 만족할 인터페이스의 **타입 SSOT** — 런타임 값 export 0(`PORT_TYPES_ONLY`). **service는 구상 repository가 아니라 이 인터페이스 타입에 의존**한다. **근거**(A1·A10, 검증 3-0): 인터페이스 의존이라야 in-memory 더블 주입으로 service 격리 테스트가 *실제로* 성립하고, ORM(drizzle 등)을 갈아끼워도 service·port가 불변(도구 중립). 헥사고날 ports & adapters의 driven port — Sairyss·Stemmler·Cockburn 만장일치의 "repository는 인터페이스로 먼저".
- **조립 패턴**: service는 팩토리(`export function makeXxx(repo: XxxPort)` — 순수, vitest 대상)와 기본 배선(`export const xxx = makeXxx(concreteRepo)` — 이 파일이 조립 지점, repository 값 import는 여기서만)을 함께 export. remote는 배선된 유스케이스만 호출(remote→service 유지). **체크리스트**(비자동): repository/adapter는 대응 port를 `implements` · service 유스케이스는 port 타입으로 repo를 받는다(구상 직결은 조립 지점에서만).
- **`*.service.ts` = 업무 규칙·트랜잭션 경계** (여러 repository 조합·adapter 소비 — **타 slice repository 조합도 합법**: 도메인 규칙 소유자가 데이터 접근을 정방향으로 조합. service→타 service·repository→타 repository 수평은 `CROSS_SLICE_SERVER_IMPORT` — 공용 능력은 server/shared 또는 인프라 선언(config `serverInfraSlices`)).
- **`*.repository.ts` = 데이터 접근**(대응 port의 구현. schema 값 import 합법처 — `SCHEMA_VALUE_OUTSIDE_REPOSITORY`. adapter의 db 클라이언트 조립도 합법, 시드도 예외 없음).
- 셋 다 SvelteKit(`$app/*`·`getRequestEvent`) import 금지(`SERVICE_SVELTEKIT_IMPORT`, `$env`는 허용하되 해석은 config 소유 권장). 위치 = `server/<slice|shared>/`(`SERVER_KIND_PLACEMENT`). `arch:new port|service|repository <slice>`가 삼단 골격 생성.

### 3.9 `*.adapter.ts` — 외부 시스템 어댑터

db 인스턴스·S3·SMTP·LLM SDK·better-auth 등 외부 세계의 래퍼(헥사고날 driven adapter). 외부 능력에 계약이 필요하면 `*.port.ts` 인터페이스를 두고 adapter가 `implements`(예: `Mailer` port ← SMTP adapter) — service는 그 port 타입으로 소비(구상 SDK 비의존). 소비 = **service·repository·adapter**만(`ADAPTER_CONSUMER`).
...
### 3.14 `shared/vendor/` — 외부 UI 킷 보존 구역 (shadcn-svelte 등)

외부 UI 킷 산출물 원본(ui 컴포넌트 + 자체 유틸) 보존, 수정 금지(재다운로드 가능해야) — **shadcn-svelte가 대표 예이나 킷 종속은 아니다**(교체 가능한 외부 구역, 도구 중립). 소비는 **shared/ui의 래핑만**(`VENDOR_IMPORT`). **Tailwind v4 전제**(현행 주류): CSS-first `@theme`, JS `tailwind.config` 선택·제거 가능, 색 토큰 OKLCH, primitive `data-slot`. **장기 방향**: 종착지는 shared/ui 흡수 — 흡수 시 cn·tv 제거·`class={[...]}` 정규화(twMerge 충돌 해소는 variant 규율이 대체 — §A9 트레이드오프 감안; 흡수 후 시각 스모크 필수). 착수 시점은 프로젝트가 결정한다.
