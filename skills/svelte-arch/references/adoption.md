...
- `.svelte-arch/`(CLI·config·템플릿) + 훅 마커 주입 + package scripts + 루트 AGENTS.md 마커 블록.
- 구 트리 감지 시 sync가 이행 필요를 알린다. **이행 전 audit·manifest는 룰 폭주 대신 plan 안내만 출력**한다.

## 1. 공식 기본 좌표 확인

`src/routes`·`src/lib`·`src/app.html`·`src/app.css`·`src/hooks.*`를 유지한다. deprecated `kit.files.*` 커스텀 좌표를 새로 만들지 않는다. v5 커스텀 좌표(`src/app/routes`, 최상위 `src/shared` 등)는 v6 마이그레이션 안내에 따라 먼저 공식 기본 좌표로 옮긴 뒤 plan을 실행한다.
...
4. **메타 동시 시딩**: 내용을 연 파일은 분류 근거를 그 자리에서 `@component` 역할 1행(view)·slice AGENTS.md 1행으로 적는다 — 읽기 비용을 두 번 내지 않고, 이행 직후 `MISSING_COMPONENT_DOC` 부채를 원천 차단한다(적용 후 이행 커밋에 포함).

## 2.7 해체 후보 (⚒ — 이동으로 해결 불가)

`.svelte`가 서버 모듈(`$lib/server/*`·`.service`·`.repository`·drizzle)을 직접 소비하면 어느 계층으로 옮겨도 audit 위반이 남는다. plan이 `⚒ 해체 후보` 목록으로 분리 보고하며, apply는 그대로 이동만 한다(이동 자체는 무해 — 부채로 계측). 처방 = **이행 커밋과 분리한 별도 승인 리팩토링**: ① 마크업 → `.view` ② 데이터 배선 → `.container` + remote(`entities/<slice>/api`) ③ 서버 로직 → `$lib/server/<slice>`의 service·repository 추출. 분할 구조 자체가 없는 프로젝트(god component 다수)는 이 라운드가 사실상 본 공사다 — 파일 단위로 제안→승인→검증을 반복한다.
...
- 완전히 깨끗한 작업트리 필수(롤백=git). 이동·리네임(`.svelte`→`.view.svelte`)·비합법 배럴 삭제·임포트 재작성(`$lib/…`·상대·배럴 named → `$lib/…` 딥 or slice index)을 한 번에. 코드 로직과 섞이지 않는 순수 구조 커밋.
...
- [ ] 계층·slice 루트 AGENTS.md 전수 + 루트 마커 블록
