...
│   ├── templates/           # arch:new 생성기 템플릿 (kit-owned)
│   └── hooks/pre-commit     # core.hooksPath 가 여기를 가리킴
├── package.json             # arch:manifest·audit·analyze·new·plan 5줄
...
- **최초 실행** = 스캐폴드: 위 풋프린트 전체 + README 씨앗(관장 트리 전 디렉토리, 역할 초안 자동 기입) + 마커 블록. **무표 컴포넌트가 감지되면 `arch:plan` 안내를 출력** — 에이전트는 플랜을 사용자에게 제시하고 "이렇게 옮기겠습니다. 진행할까요?" **승인 후에만** `--apply`(SKILL.md §온보딩 규범).
