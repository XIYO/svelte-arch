...
- **Svelte 검증**: 지원 호스트에서는 설치된 `svelte-arch` 플러그인의 공용 Svelte MCP로 `.svelte` 변경 전후 `svelte_autofixer`를 확인한다. 같은 호스트에 같은 endpoint를 프로젝트 MCP 설정으로 중복 등록하지 않는다. 마지막에는 프로젝트 전체 `bun run check`로 컴파일·타입·a11y를 확인하며, MCP 미연결도 이 검사는 생략하지 않는다.
