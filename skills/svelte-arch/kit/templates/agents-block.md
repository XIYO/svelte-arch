...
- **4단 주소**: `계층/slice/segment/접미사`. 계층은 `shared·entities·features·widgets·pages` + 병렬 `server`, segment는 `ui·api·model·lib·config`다. `.view`=dumb, `.container`=remote 배선·마크업 0, `.remote/.service/.repository/.port/.adapter/.guard/.schema/.config/.util`은 역할을 파일명으로 선언한다.
...
- **코딩 게이트**: 새 Svelte는 runes만 쓴다(`$props`·`$state`·`$derived`·`$effect`). `export let`·`$:`·`on:`·`createEventDispatcher`·`<slot>`·`$$props`·`$app/stores`·`beforeUpdate/afterUpdate`를 쓰지 않는다. event는 `onclick` 등 속성, 컴포넌트 통신은 `onXxx` callback prop, 콘텐츠 주입은 Snippet+`{@render}`다.
