...
- **Svelte 검증**: `.svelte` 변경은 연결된 공식 Svelte MCP의 `svelte_autofixer`로 전후 확인하고, 마지막에는 프로젝트 전체 `bun run check`로 컴파일·타입·a11y를 확인한다. MCP 미연결 환경도 `bun run check`는 생략하지 않는다.
