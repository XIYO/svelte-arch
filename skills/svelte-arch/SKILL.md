...
3. **3차(해체)** — plan의 `⚒ 해체 후보`(.svelte의 서버 직접 소비)는 이동으로 해결 불가. **이행 커밋과 분리**해 별도 승인 후 view/live 분리·server 3계층 추출 리팩토링. adoption.md §2.7.
...
| 감사 룰 50 전량 (steiger 흡수분 인라인 표기) | `references/audit-rules.md` |
